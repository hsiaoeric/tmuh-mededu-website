    ...
    return (
      <button
        ...
        style={{
          ...
          color: on ? 'var(--on-accent)' : 'var(--body)',
        }}
      >
        {label}
      </button>
...
export function DeptView() {
  ...
  const activeKpiPeople = activeKpiGroup
    ? (KPI_MEMBER_GROUPS[activeKpiGroup] ?? []).map((p) =>
        resolvePerson(p, activeKpiGroup === 'Teaching Attendings' ? 'var(--tone-gold)' : 'var(--tone-slate)', lang),
      )
    : [];
        ...
        >
          ...
          <div style={{ position: 'relative', padding: 'clamp(34px,5vw,72px)', maxWidth: 760 }}>
            ...
            >
              {t.heroTitle1}
              <br />
              <span style={{ color: 'var(--on-ink-accent)' }}>{t.heroTitle2}</span>
            </h1>
            <div style={{ width: 80, height: 3, borderRadius: 'var(--r-xs)', background: 'var(--on-ink-accent)', margin: '26px 0 22px' }} />
            <p style={{ fontSize: 18, lineHeight: 1.85, color: 'var(--on-ink-dim)', maxWidth: 560 }}>{t.heroTag}</p>
          ...
          {contacts.map((c, i) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <span style={{ fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: 14.5, color: 'var(--text)' }}>
                  {c.center}
                </span>
              </div>
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: 14, color: 'var(--body)', marginBottom: 7 }}>
                {c.person}
              </div>
              <div
                style={{
