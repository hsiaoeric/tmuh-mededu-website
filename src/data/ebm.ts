...
/*
 * The center's accents, as theme-aware tokens rather than fixed hex.
 *
 * They are consumed by interpolating into inline styles and `color-mix()`, both
 * of which accept `var()`, so the light/dark pair defined in tokens.css applies
 * without every call site needing to know the theme. `ink` stays literal — it
 * is the deep ground of the hero panel, which is dark in both themes.
 */
...
export function buildEbm(lang: Lang) {
  ...
  const kpis: EbmKpi[] = [
    ...
    { num: 15, suffix: '+', zh: '年 NHQA 持續參與', en: 'Years at NHQA', color: 'var(--c-research)' },
    { num: 3, suffix: '', zh: '大競賽組別', en: 'Contest Tracks', color: 'var(--c-facdev)' },
  ...
  const awardsLit: EbmAward[] = [
    { sess: pick(lang, '第 7 屆', '7th'), award: pick(lang, '銀獎', 'Silver'), tone: 'var(--c-admin)' },
    { sess: pick(lang, '第 14 ・15 屆', '14th · 15th'), award: pick(lang, '潛力獎', 'Potential'), tone: 'var(--c-research)' },
  ...
  const awardsClin: EbmAward[] = [
    { sess: pick(lang, '第 10 屆', '10th'), award: pick(lang, '潛力獎', 'Potential'), tone: 'var(--c-research)', note: '' },
    ...
    { sess: pick(lang, '第 12 屆', '12th'), award: pick(lang, '潛力獎', 'Potential'), tone: 'var(--c-research)', note: pick(lang, '預防性投與類固醇以改善全身麻醉手術後之冑心嘔吐專案', 'Prophylactic steroids to reduce post-anesthesia nausea & vomiting') },
  ];

  const awardsTrans: EbmAward[] = [
    { sess: pick(lang, '113 學年起', 'From AY113'), award: pick(lang, '新增參與', 'Newly Joined'), tone: 'var(--c-skills)', note: pick(lang, '自 113 學年度起參與 NCMEA 知識轉譯組競賽，拓展實證應用新面向。', 'Joined the NCMEA Knowledge-Translation track from AY113, opening a new dimension of evidence application.') },
    { sess: pick(lang, '推動重點', 'Focus'), award: pick(lang, '證據轉化', 'Translation'), tone: 'var(--c-research)', note: pick(lang, '以每月「實證醫學知識轉譯討論會」為基礎，推動證據轉化為臨床決策。', 'Built on the monthly knowledge-translation forum, turning evidence into clinical decisions.') },
  ];
  ...
  const courseGroups: EbmCourseGroup[] = [
    ...
    { gicon: 'cap', title: pick(lang, '師資培育', 'Faculty Cultivation'), color: 'var(--c-research)', rows: pick(lang, [['實證醫學種子師資培訓課程', '每年 3–4 堂'], ['院內實證教學人才庫', '系統性管理'], ['培養未來實證領導者', '長期培育計畫']], [['Seed-teacher training', '3–4 sessions/year'], ['Teaching talent pool', 'Systematic management'], ['Future EBM leaders', 'Long-term program']]).map((r) => ({ name: r[0], detail: r[1] })) },
  ];
